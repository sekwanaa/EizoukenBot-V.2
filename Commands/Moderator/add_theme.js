...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("month").setDescription("Choose a month to add/update a theme for").setRequired(true)
    )
    .addStringOption((option) =>
  ...
  async execute(interaction) {
    ...
    if (year) {
      const action = await themesData.addThemeData(month, year, theme);
